...
int main (int argc, char* argv[]) {
	...
	for (iterator = 1; iterator < argc; iterator++) {
		...
		while (ptr) {
			...
			state.code_table[ref->code_address] = (find_code_label(&state, ref->name)->address + MEM_STARTS_AT) << 2 | RELOCATABLE;
			ptr = ptr->next;
		}
		...
		free_list(state.entry_table, free_entry_with_line_num);
		free_list(state.extern_table, free);
		free_list(state.extern_refs_table, free_refs);
		free_list(state.code_label_refs_table, free_refs);
